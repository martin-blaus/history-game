...
async function checkUrl(url) {
  ...
  try {
    ...
    return getRes.status === 200 || getRes.status === 301 || getRes.status === 302;
  } catch (e) {
    return false;
  }
}
  ...
  for (let i = 0; i < deck.events.length; i++) {
    ...
    if (!hasLink || options.forceUpdate) {
      ...
      if (!searchTitle && event.people && event.people.length > 0) {
        console.log(`  No search result for event. Trying person: "${event.people[0]}"...`);
      ...
      if (searchTitle) {
        ...
        console.log(`  Proposed URL: ${Colors.green}${canonicalUrl}${Colors.reset}`);
        ...
        if (isValid) {
          ...
          if (options.fix) {
            ...
            modified = true;
            fixedCount++;
            console.log(`    ${Colors.bright}${Colors.green}[UPDATED]${Colors.reset} Set wikipediaUrl for "${event.event}"`);
          } else {
            console.log(`    [DRY RUN] Would update "${event.event}" with wikipediaUrl: ${canonicalUrl}`);
          }
        } else {
          console.log(`    Status: ${Colors.red}INVALID URL (cannot resolve)${Colors.reset}`);
          unmatchedList.push({ deck: deckName, event: event.event, reason: 'Constructed URL failed resolution checks' });
        }
      } else {
        console.log(`    ${Colors.yellow}No Wikipedia article found for "${event.event}"${Colors.reset}`);
        unmatchedList.push({ deck: deckName, event: event.event, reason: 'No matching Wikipedia article found' });
      }
...
Options:
  ...
  console.log(`${Colors.bright}Starting Wikipedia Link Search & Validation Utility${Colors.reset}`);
  ...
  console.log(`${Colors.bright}${Colors.magenta}               FINAL UNMATCHED REPORT               ${Colors.reset}`);
  console.log(`${Colors.bright}${Colors.magenta}====================================================${Colors.reset}`);
  if (unmatchedList.length === 0) {
    console.log(`${Colors.green}All events across all processed decks were successfully matched and linked to Wikipedia!${Colors.reset}`);
  } else {
    console.log(`${Colors.yellow}The following ${unmatchedList.length} events could not be matched to a valid Wikipedia URL:${Colors.reset}`);
    unmatchedList.forEach(item => {
      console.log(`  - [${item.deck}] "${item.event}" -> ${Colors.red}${item.reason}${Colors.reset}`);
    });
  }
}
